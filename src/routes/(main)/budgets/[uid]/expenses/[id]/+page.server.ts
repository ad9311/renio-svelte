...
export const load: PageServerLoad = async ({ fetch, params }) => {
	const response = await getResource(`${PUBLIC_API}/budgets/${params.uid}/expenses/${params.id}`, {
		fetch,
	});
	if (response.ok) {
		const json = await response.json();
		return { expense: json.data.expense };
	}
...
export const actions = {
	editExpense: async ({
		...
		const response = await patchResource(
			`${PUBLIC_API}/budgets/${params.uid}/expenses/${formData.get('expense_id')}`,
			{
				sessionToken,
				body,
			},
	...
	deleteExpense: async ({
		...
		const response = await deleteResource(
			`${PUBLIC_API}/budgets/${params.uid}/expenses/${params.id}`,
			{ fetch },
		);
		if (response.ok) {
